...
for (p = 5024; p < indexes.big5.length; p++)
...
function utf8Encoder(stream)
{
    ...
    while (cps.length > 0)
    {
        ...
        if (cp >= 0x80 && cp <= 0x7FF) { count = 1;
            offset = 0xC0; }
        else if (cp >= 0x800 && cp <= 0xFFFF) { count = 2;
            offset = 0xE0; }
        else { count = 3;
            offset = 0xF0; }
        var bytes = []
...
function iso2022jpDecoder(stream)
{
    ...
    while (!finished)
    {
        ...
        {
            case 'ascii':
                if (byte == 0x1B) { decState = 'escStart'; continue }
            ...
            case 'leadbyte':
                ...
                break
            case 'trailbyte':
                if (byte == 0x1B) { decState = 'escStart';
                    out += '�'; continue }
                else if (byte >= 0x21 && byte <= 0x7E)
                {
                    decState = 'leadbyte'
                ...
                else if (byte == endofstream) { decState = 'leadbyte';
                    bytes.unshift(byte);
                    out += '�'; continue }
                else { decState = 'leadbyte';
                    out += '�'; continue }
                break
            case 'escStart':
